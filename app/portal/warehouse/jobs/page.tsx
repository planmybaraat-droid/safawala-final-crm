...
export default function WarehouseJobsPage() {
  ...
  // Picking-only queue: warehouse task in waiting or in_progress.
  const pickingQueue = jobs.flatMap((job) =>
    (job.job_tasks || [])
      .filter((t) => t.department === "warehouse" && (t.status === "waiting" || t.status === "in_progress"))
      .map((task) => ({ job, task }))
  )
  ...
  async function handleOpenTask(job: Job, task: JobTask) {
    ...
    if (list.length === 0) {
      try {
        ...
        list = items.map((it: any) => ({ text: `${it.product_name || it.name || "Item"} x${it.quantity || 1}`, checked: false }))
      } catch {
        // no items available — leave checklist empty, picking can still complete
      }
    }
    setChecklist(list)
  }
  ...
  return (
    <div className="pb-6">
      <PortalPageHeader title="Warehouse Jobs" subtitle="Picking queue" color={COLOR} backHref="/portal/warehouse" />
      ...
      <PortalSectionLabel label={`Picking Jobs (${pickingQueue.length})`} />
      ...
      <div className="mx-4 rounded-2xl overflow-hidden shadow-sm" style={{ background: "rgba(255,255,255,0.65)", border: "1px solid rgba(255,255,255,0.9)" }}>
        {loading ? (
          ...
          <PortalEmptyState icon="package" title="No jobs to pick" subtitle="No active picking jobs right now." color={COLOR} />
        ) : (
          pickingQueue.map(({ job, task }) => (
            <PortalListCard
              key={task.id}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white rounded-t-[28px] sm:rounded-3xl w-full max-w-lg max-h-[85vh] sm:max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-100">
            <div className="px-6 py-4 border-b flex justify-between items-center bg-slate-50 sticky top-0 z-10">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-purple-600">{selectedJob.job_number}</span>
            ...
            <div className="p-6 space-y-5">
              {checklist.length > 0 && (
                <div className="space-y-2">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Items to Pick</p>
                  <div className="space-y-2">
              ...
              <div className="pt-2 space-y-2">
                {selectedTask.status === "waiting" && (
                  <button onClick={() => updateStatus("in_progress")} disabled={updating}
                    ...
                    {updating ? "Starting…" : "▶ Start Picking"}
                  </button>
                )}
                <button onClick={() => updateStatus("completed")} disabled={updating}
                  className="w-full py-3.5 rounded-xl text-[13px] font-bold text-white transition-opacity"
                  style={{ background: "#22c55e", opacity: updating ? 0.7 : 1 }}>
                  {updating ? "Completing…" : "✓ Complete Picking"}
                </button>
              </div>
            </div>
          </div>
