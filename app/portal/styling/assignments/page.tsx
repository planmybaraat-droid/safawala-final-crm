...
export default function AssignmentsPage() {
  ...
  async function handleSubmitInterest(e: React.FormEvent) {
    ...
    try {
      ...
      // Resolve the Job created for this booking, then register interest against
      // the new job_interests table (role inferred as 'styling').
      const jobRes = await fetch(`/api/jobs?booking_id=${modal.booking.id}`)
      const jobData = await jobRes.json()
      const job = Array.isArray(jobData.data) ? jobData.data[0] : null

      if (job?.id) {
        const res = await fetch(`/api/jobs/${job.id}/interest`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ role: "styling", note: `Rate: ${payload.quoted_rate}. ${payload.notes || ""}`.trim() }),
        })
        const data = await res.json()
        ...
        // Fall back to the legacy endpoint if no Job exists yet for this booking.
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", minHeight: "100vh", background: "#fdf2f8", paddingBottom: 40 }}>
      ...
      {errorState && (
        <div className="mx-4 mt-4 p-4 bg-red-50 border border-red-200 rounded-2xl flex flex-col gap-1.5 shadow-sm">
          <p className="text-[12px] font-extrabold text-red-800 flex items-center gap-1.5">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> Connection Notice
          </p>
      ...
      {successToast && (
        <div className="mx-4 mt-3 p-3.5 rounded-2xl text-xs font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 shadow-sm flex items-center gap-2 animate-bounce">
          <span style={{ display: "inline-flex" }}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22,4 12,14.01 9,11.01"/></svg></span>
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <h3 className="text-xs font-black text-gray-900 mt-1 flex items-center gap-1">
                    ...
                    <div className="bg-pink-50/40 p-2.5 rounded-xl border border-pink-100/60 mt-0.5">
                      <p className="text-[10px] text-gray-700 font-semibold leading-relaxed flex items-start gap-1">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: 1 }}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                        <span><strong>Venue / Address:</strong> {address}</span>
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px] font-bold text-gray-600 border-t border-dashed border-pink-100 pt-2 mt-0.5">
                      <div>
                        ...
                        <span className="inline-flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> {eventDate ? new Date(eventDate).toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short", year: "numeric" }) : "—"}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-gray-400 block font-semibold uppercase">Time & Volume</span>
                        <span>⏰ {eventTime} • <span className="inline-flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27,6.96 12,12.01 20.73,6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg> {safaCount} Safas</span></span>
                      </div>
                    </div>
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <h3 className="text-xs font-bold text-gray-800 mt-1 flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> {customerName}</h3>
                      </div>
                      <span className="text-[9px] font-extrabold px-2.5 py-1 rounded-full uppercase bg-emerald-100 text-emerald-800 flex items-center gap-1">
                        Interest Registered <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20,6 9,17 4,12"/></svg>
                      </span>
                    </div>
                    ...
                    <div className="bg-emerald-50/50 p-2.5 rounded-xl border border-emerald-100 mt-1 text-xs font-bold text-emerald-900 space-y-1">
                      <p className="flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> <strong>Artist:</strong> {interest.stylist_name} ({interest.stylist_phone})</p>
                      <p className="flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="6" y1="3" x2="18" y2="3"/><line x1="6" y1="8" x2="18" y2="8"/><line x1="6" y1="8" x2="13" y2="21"/><path d="M6 3h8a4 4 0 0 1 0 5H6"/></svg> <strong>Interested Rate / Fee:</strong> ₹{interest.quoted_rate}</p>
                      {interest.notes && <p className="text-[10px] text-gray-600 font-normal flex items-center gap-1"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> Notes: {interest.notes}</p>}
                    </div>
                  </div>
                )
              })}
