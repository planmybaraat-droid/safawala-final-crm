...
interface Product  { id:string; name:string; product_code:string; category:string; sale_price:number; image_url?:string; stock_available?:number; category_id?:string }
...
function NewSaleInner() {
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      const { data: categoriesData } = await supabaseClient.from('product_categories').select('*')
      const categoryMap: { [key: string]: string } = {}
      if (categoriesData) categoriesData.forEach((c: any) => { categoryMap[c.id] = c.name })
      const mapped: Product[] = productsWithBarcodes
        .filter((p: any) => p.name && Number((p as any).price || (p as any).sale_price) > 0)
        .map((p: any) => ({
          id: p.id, name: p.name, product_code: p.product_code || '',
          category: p.category_id ? (categoryMap[p.category_id] || '') : '', category_id: p.category_id,
          sale_price: (p as any).price || (p as any).sale_price || 0,
          image_url: (p as any).image_url || undefined,
          stock_available: p.stock_available ?? 0,
        }))
      setProducts(mapped)
    } catch (e) { console.warn("[New Sale] Failed to load products:", e) }
  ...
  useEffect(() => { if(step===2) loadProducts() }, [step, loadProducts])
  useEffect(() => { if(step===3) loadStaff() }, [step, loadStaff])
  useEffect(() => () => stopScan(), [])
  ...
  function addToCart(product: Product) {
    setCart(c=>{
      ...
      return [...c, { product, quantity:1, unit_price:product.sale_price }]
    })
  }
  ...
  async function lookupAndAddBarcode(code: string) {
    ...
    try {
      ...
      if (product) {
        addToCart({ id: product.id, name: product.name, product_code: product.product_code||'', category: product.category||'', sale_price: product.price||product.sale_price||0, image_url: product.image_url, stock_available: product.stock_available })
        toast.success(`${product.name} added to cart`)
        setShowScanModal(false)
      } else setScanError("No product found for this barcode.")
    } catch { setScanError("Lookup failed. Try again.") }
  }
  async function startScan() {
    setScanError("")
    if (!("BarcodeDetector" in window)) { setScanError("This browser doesn't support camera scanning. Use manual entry."); return }
    try {
      ...
      const tick = async () => {
        ...
        try {
          ...
          if (codes.length > 0) { const value = codes[0].rawValue; stopScan(); lookupAndAddBarcode(value); return }
        } catch {}
        rafRef.current = requestAnimationFrame(tick)
      }
      rafRef.current = requestAnimationFrame(tick)
    } catch (e:any) { setScanError(e?.name === "NotAllowedError" ? "Camera access denied." : "Couldn't access the camera.") }
  }
  ...
  async function saveSale() {
    ...
    try {
      ...
      const res = await fetch("/api/direct-sales", {
        ...
        body: JSON.stringify({
          sale: {
            ...
            payment_type: balance<=0 ? 'full' : 'advance',
            ...
            status: balance<=0 ? 'confirmed' : 'pending_payment',
            notes,
            sales_closed_by_id: salesStaffId || null,
          },
          items: cart.map(i=>({
      ...
      router.push(`/portal/booking/bookings/${data.data.sale.id}?kind=sale`)
    } catch(e:any) { setSaveError(e.message); setSaving(false) }
  }

  const canGoStep2 = !!selectedCustomer
  const canSave = !!selectedCustomer && cart.length>0
  const stepLabels = ["Customer","Products","Review"]

  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#FFF3E8 0%,#FFF3E8 100%)", fontFamily:"var(--font-inter), Inter, sans-serif" }}>
      <div style={{ background:`linear-gradient(135deg,${COLOR_DARK},${COLOR})`, padding:"20px 16px 16px", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
          ...
          <div style={{ flex:1 }}>
            <h1 style={{ color:"white", fontSize:18, fontWeight:900, margin:0 }}>🛍️ New Sale</h1>
            <p style={{ color:"rgba(255,255,255,0.65)", fontSize:11, margin:0 }}>{stepLabels[step-1]} — Step {step} of 3</p>
          </div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:0 }}>
          {[1,2,3].map((n,i)=>(
            <div key={n} style={{ display:"flex", alignItems:"center", flex:i<2?1:"none" }}>
              ...
              {i<2 && <div style={{ flex:1, height:2, background:n<step?"rgba(255,255,255,0.8)":"rgba(255,255,255,0.25)", margin:"0 4px" }} />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding:"16px 16px 120px" }}>
              ...
              <div>
                ...
                {!showNewCust && (
                  <button onClick={()=>setShowNewCust(true)}
                    style={{ width:"100%", padding:"12px 0", borderRadius:14, border:`1.5px dashed ${COLOR}`, background:"#FFF3E8", color:COLOR_DARK, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit", marginBottom:10 }}>
                    + Add New Customer
                  </button>
                )}
        ...
        {step===2 && (
          <div>
            ...
            <div style={{ display:"flex", gap:8, marginBottom:12 }}>
              ...
              <button onClick={() => { setShowScanModal(true); setScanError("") }}
                style={{ width:46, height:46, borderRadius:14, border:"none", background:"#1e1208", color:"white", cursor:"pointer", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center" }}
                title="Scan barcode">
                  ...
                  return (
                    <div key={p.id} style={{ background:"white", borderRadius:14, padding:"12px 14px", display:"flex", alignItems:"center", gap:12, opacity:outOfStock?0.55:1 }}>
                      ...
                      <div style={{ textAlign:"right", flexShrink:0, marginLeft:8 }}>
                        <p style={{ margin:"0 0 6px", fontSize:13, fontWeight:800, color:COLOR_DARK }}>{fmt(p.sale_price)}</p>
                        {inCart ? (
                          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
        ...
        {step===3 && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ background:"white", borderRadius:18, padding:16, border:"1.5px solid rgba(249,115,22,0.15)" }}>
              ...
              <select value={salesStaffId} onChange={e=>setSalesStaffId(e.target.value)}
                style={{ width:"100%", padding:"11px 12px", borderRadius:12, border:"1.5px solid rgba(249,115,22,0.25)", fontSize:13, fontWeight:700, color:"#1e1208", outline:"none", fontFamily:"inherit", background:"#FFFAF5" }}>
            ...
            <div style={{ background:"white", borderRadius:18, padding:16 }}>
              <p style={{ margin:"0 0 12px", fontSize:10, fontWeight:700, color:"rgba(80,55,30,0.4)", letterSpacing:1.2, textTransform:"uppercase" }}>Sale Summary</p>
              {[
            ...
            <div style={{ background:"white", borderRadius:18, padding:16 }}>
              ...
              <div style={{ padding:"10px 0", borderBottom:"1px solid rgba(0,0,0,0.04)" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                  <span style={{ fontSize:11, color:"rgba(80,55,30,0.5)", fontWeight:600 }}>Amount Paid (₹)</span>
                ...
                <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                  {PAYMENT_METHODS.map(m=>(
                    <button key={m} onClick={()=>setPaymentMethod(m)}
                      style={{ padding:"5px 12px", borderRadius:16, border:`1.5px solid ${paymentMethod===m?COLOR:"rgba(0,0,0,0.08)"}`, background:paymentMethod===m?"#FFF3E8":"white", color:paymentMethod===m?COLOR_DARK:"rgba(80,55,30,0.5)", fontSize:11, fontWeight:paymentMethod===m?700:500, cursor:"pointer", fontFamily:"inherit" }}>
                      {m}
                    </button>
                  ))}
                </div>
            ...
            <button onClick={saveSale} disabled={saving||!canSave}
        ...
        <div style={{ position:"fixed", inset:0, zIndex:100, background:"rgba(0,0,0,0.6)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
          <div style={{ width:"100%", maxWidth:420, background:"white", borderRadius:24, padding:20, boxShadow:"0 20px 40px rgba(0,0,0,0.2)" }}>
            ...
            <div style={{ marginTop:14 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <input type="text" id="manual-barcode-input-sale" placeholder="Type barcode…"
                ...
                <button onClick={()=>{ const el=document.getElementById('manual-barcode-input-sale') as HTMLInputElement; if(el) lookupAndAddBarcode(el.value) }}
      ...
      <div style={{ position:"fixed", bottom:0, left:0, right:0, background:"white", borderTop:"1px solid rgba(0,0,0,0.06)", padding:"12px 16px calc(env(safe-area-inset-bottom,0px) + 12px)", display:"flex", gap:10, zIndex:50 }}>
        ...
        {step<3 && (
          <button onClick={()=>setStep(s=>s+1)} disabled={step===1?!canGoStep2:false}
            style={{ flex:2, height:48, borderRadius:14, border:"none", background:(step===1&&!canGoStep2)?"#e5e7eb":`linear-gradient(135deg,${COLOR},${COLOR_DARK})`, color:(step===1&&!canGoStep2)?"#9ca3af":"white", fontSize:14, fontWeight:800, cursor:(step===1&&!canGoStep2)?"not-allowed":"pointer", fontFamily:"inherit" }}>
            Next →
          </button>
        )}
      </div>
...
export default function NewSalePage() {
  return (
    <Suspense fallback={
      <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#FFF3E8,#FFF3E8)", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <div style={{ width:40, height:40, borderRadius:"50%", border:"3px solid #f9731630", borderTopColor:"#f97316", animation:"spin 1s linear infinite" }} />
        <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      </div>
    }>
      <NewSaleInner />
