...
# Insert all Safa products via API
# This script sends a POST request to bulk-insert products
...
echo "🚀 Inserting 87 Safa products via API..."
echo "Request will be sent to: http://localhost:3000/api/products/bulk-insert"

ROLE_KEY="${SUPABASE_SERVICE_ROLE_KEY}"
...
curl -X POST http://localhost:3000/api/products/bulk-insert \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ROLE_KEY" \
  -d "{
    \"products\": $PRODUCTS,
    \"franchiseId\": \"5322e8d9-d4d8-4c48-8563-c9785c1cffd0\",
    \"categoryName\": \"Safa\"
  }"
